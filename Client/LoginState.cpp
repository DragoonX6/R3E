...
int LoginState::HandleEvent(GameEvent* gevt){
	if(gevt->_evt_type == GE_LOGIN){
		...
		//LMA: possibly coming from "START" button so don't rely on it since it replyes -1 as avatar selected...
		char selected_avatar=0;
		if(evt->mCharacterIdx!=-1){
			selected_avatar=char(evt->mCharacterIdx);
			mSelectedCharacter=evt->mCharacterIdx;
		}
		else{
		...
		LOG("Player selected avatar %s",mAvatars[selected_avatar].mName.Str());
		pakout.Add(mAvatars[selected_avatar].mName);
		gNetwork->SendPacket(WS, &pakout);
